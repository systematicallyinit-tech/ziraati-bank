...
export const GET = async (req) => {
  try {
    ...
    const transactions = await Nft.find().sort({ createdAt: -1 });
...
export const DELETE = async (req) => {
  try {
    ...
    const deletedTransaction = await Nft.findOneAndDelete(
      { _id: id },
    );
...
export const PATCH = async (req) => {
  try {
    ...
    // Update user's transaction status
      const updatedTransaction = await Nft.findOneAndUpdate(
        { _id: transactionID },
        { status },
        { new: true }
      );
