...
export const GET = async (req) => {
  ...
  try {
    ...
    const transactions = await Transaction.find({ transaction_type: type }).populate("userId").sort({ createdAt: -1 });
...
export const PATCH = async (req) => {
  try {
    ...
    if(updatedTransaction.transaction_type === "deposit" && status === "Successful") {
      const updatedUserBalance = await User.findOneAndUpdate(
        { _id: userId },
        { $inc: { balance: amount, deposit: amount, total_deposit: amount } },
        { new: true }
      );
    ...
    if(updatedTransaction.transaction_type === "deposit" && status === "Pending") {
      const updatedUserBalance = await User.findOneAndUpdate(
        { _id: userId },
        { $inc: { balance: -amount, deposit: -amount, total_deposit: -amount } },
        { new: true }
    ...
    if (updatedTransaction.transaction_type === "withdrawal" && status === "Successful") {
      const updatedUserBalance = await User.findOneAndUpdate(
        { _id: userId },
        { $inc: { balance: -amount, earnings: -amount, withdrawal: amount, total_withdrawal: amount } },
        { new: true }
      );
      ...
      const now = new Date();
        ...
        let mailOptions = {
          from: `support@${process.env.NEXT_PUBLIC_COMPANY_NAME_SMALL} <${process.env.SMTP_USERNAME}>`,
          to: `${updatedUserBalance.email}`,
          subject: `Withdrawal Approval - ${process.env.NEXT_PUBLIC_COMPANY_NAME}`,
          text: `Your withdrawal of ${updatedTransaction.amount} ${updatedTransaction.currency} has been successfully Approved. Thank you for using ${process.env.NEXT_PUBLIC_COMPANY_NAME}. ${formatter.format(
            now
          )}`,
          html: "",
        };
    ...
    if (updatedTransaction.transaction_type === "withdrawal" && status === "Pending") {
      const updatedUserBalance = await User.findOneAndUpdate(
        { _id: userId },
        { $inc: { balance: amount, earnings: amount, withdrawal: -amount, total_withdrawal: -amount } },
        { new: true }
      ...
      const now = new Date();
        ...
        let mailOptions = {
          from: `support@${process.env.NEXT_PUBLIC_COMPANY_NAME_SMALL} <${process.env.SMTP_USERNAME}>`,
          to: `${updatedUserBalance.email}`,
          subject: `Withdrawal Rejection - ${process.env.NEXT_PUBLIC_COMPANY_NAME}`,
          text: `Your withdrawal of ${updatedTransaction.amount} ${updatedTransaction.currency} has been rejected. Thank you for using ${process.env.NEXT_PUBLIC_COMPANY_NAME}. ${formatter.format(
            now
          )}`,
          html: "",
        };
